            ...
            <div class="card">
                ...
                <Toolbar class="mb-4">
                    <template v-slot:start>
                        <div class="my-2">
                            <span class="block p-1 text-black-alpha-90 text-5xl">Kategori</span>
                            <span class="block lb-desc p-1">View all kategori</span>
                        </div>
                    </template>
                    <template v-slot:end>
                        <div class="my-2">
                ...
                <DataTable v-else-if="store.categories && store.categories.length > 0 && !loadingData"
                    class="p-datatable-gridlines p-datatable-lg" ref="dt" :value="store.categories" :paginator="true"
                    ...
                    <Column header="Action" headerStyle="min-width:10rem;">
                        <template #body="slotProps">
                            ...
                            <Button icon="pi pi-trash" label="Delete" class="p-button-rounded p-button-danger mt-2"
                ...
                <Dialog :closable="false" v-model:visible="productDialog" :style="{ width: '450px' }"
                    ...
                    <div class="field">
                        ...
                        <InputText id="name" v-model.trim="product.namaKategori" required="true" autofocus
                    ...
                    <template #footer>
                        ...
                        <Button v-if="product.namaKategori" label="Save" :loading="loading" icon="pi pi-check"
                            class="p-button-text" @click="addCategory(product.namaKategori)" />
                        <Button v-else label="Add Category Name" :loading="loading" icon="pi pi-check" class="p-button-text"
                            disabled />
                    </template>
                </Dialog>
                ...
                <Dialog :closable="false" v-model:visible="updateProductDialog" :style="{ width: '450px' }"
                    ...
                    <template #footer>
                        ...
                        <Button label="Update" :loading="loading" icon="pi pi-check" class="p-button-text"
                            @click="updateCategory(product._id, product.namaKategori)" />
                    </template>
                </Dialog>
                ...
                <Dialog v-model:visible="deleteProductDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
                    <div class="flex align-items-center justify-content-center">
                        ...
                        <span v-if="product">Are you sure you want to delete <b>{{ product.namaKategori }}</b>?</span>
                    </div>
                    <template #footer>
                        <Button label="No" icon="pi pi-times" class="p-button-text" @click="deleteProductDialog = false" />
                        <Button :loading="loading" label="Yes" icon="pi pi-check" class="p-button-text"
                            @click="deleteCategory(product._id, product.namaKategori)" />
                    </template>
                </Dialog>
